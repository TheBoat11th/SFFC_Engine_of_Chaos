...
if(allow_interaction){//if we are able to use this menu
    ...
    else{//we're mapping controls
        if(display_type = "Control_Config"){
            if(Pressed_Key("Any_Button",false)){//if we pressed any button (specifically for control config)
                input_event = "Any_Button";//run select event
                event_perform(ev_other,ev_user0);//run event 1 (user input)
            }
        }
        else{
            if(Pressed_Key("Up_Button",auto_move)){//if up button
                input_event = "Up_Button";//run select event
                event_perform(ev_other,ev_user0);//run event 1 (user input)
            }
            else if(Pressed_Key("Down_Button",auto_move)){//if up button
                input_event = "Down_Button";//run select event
                event_perform(ev_other,ev_user0);//run event 1 (user input)
            }
            else if(Pressed_Key("Left_Button",auto_move)){//if up button
                input_event = "Left_Button";//run select event
                event_perform(ev_other,ev_user0);//run event 1 (user input)
            }
            else if(Pressed_Key("Right_Button",auto_move)){//if up button
                input_event = "Right_Button";//run select event
                event_perform(ev_other,ev_user0);//run event 1 (user input)
            }
            else if(Pressed_Key("Select_Button",false)){//if up button
                input_event = "Select_Button";//run select event
                event_perform(ev_other,ev_user0);//run event 1 (user input)
            }
            else if(Pressed_Key("Cancel_Button",false)){//if up button
