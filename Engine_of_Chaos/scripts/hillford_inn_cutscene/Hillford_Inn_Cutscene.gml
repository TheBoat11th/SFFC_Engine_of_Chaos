function AU_Hillford_Inn_Cutscene() {
	var _jason = player_trigger;
	var _max = Get_Character("AU_Max");
	var _ciera = Get_Character("AU_Ciera");
	var _kira = Get_Character("AU_Kira");
	var _karina = Get_Character("AU_Karina");
	
	var _bartender = Get_Character("AU_Hillford_Innkeeper");
	var _beer = noone;
	...
	switch(state){
		case 0:
	        state += 1;//Note, Max and ciera will be mid-movement at this point, so each will trigger this event almost back to back
